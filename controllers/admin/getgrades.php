...
$sql = "SELECT tbl_studentgrades.id, tbl_studentgrades.subject_codeno,
		...
		WHERE tbl_studentgrades.studentid_no = :studentid_no AND tbl_subjects.is_active = :is_active GROUP BY tbl_studentgrades.id
...
while ($row = $result->fetch(PDO::FETCH_OBJ)) {
	...
	$data[] = array(
		...
		"option" => "<button id='$id' type='button' class='btn btn-primary btn-xs btn-edit'
						data-toggle='collapse' data-parent='#accordion' data-target='#inputdiv'>
						Edit Grade
					 </button>"
	);
}
